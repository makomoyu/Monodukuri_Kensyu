...
test_image_path = r".\03_ref_roi_crop.jpg"
kinoko_template_image_path = r".\kinoko_template2.bmp"
...
class ORBJudge:
    ...
    @staticmethod
    def result():
        target_img = cv2.imread(test_image_path) # 検査画像から「きのこの山」ロゴ周辺を切り出した画像
        reference_img = cv2.imread(kinoko_template_image_path) #

        if target_img is None:
            raise FileNotFoundError(f"検査画像が読み込めません: {test_image_path}")

        if reference_img is None:
            raise FileNotFoundError(f"テンプレート画像が読み込めません: {kinoko_template_image_path}")
